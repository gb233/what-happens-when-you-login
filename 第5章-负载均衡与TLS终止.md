...
不推荐（弱或已破解）：
  N TLS_RSA_WITH_AES_256_CBC_SHA256   (静态RSA，无前向保密)
  N TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA (CBC模式，BEAST/Lucky13漏洞)
  N TLS_RSA_WITH_3DES_EDE_CBC_SHA     (3DES，SWEET32漏洞)
  N 任何包含 NULL、EXPORT、anon、RC4、MD5 的套件
```
...
```
SSL Labs满分（A+）要求：
  Y TLS 1.2+ only（禁用TLS 1.0/1.1和SSL 3.0）
  Y 无已知漏洞（Heartbleed、POODLE、BEAST等）
  Y 支持前向保密（ECDHE密钥交换）
  Y 启用HSTS且有效期 >= 6个月
  Y 证书有效，链路完整
  Y 支持SNI
...
禁止的协议（立即停用）：
  N SSL 2.0
  N SSL 3.0
  N TLS 1.0（2024年前必须停用）
  N TLS 1.1（2024年前必须停用）

要求（2024年后强制）：
  Y TLS 1.2+ only
  Y 强密码套件（不含RC4, 3DES, NULL, EXPORT）
  Y 有效证书（未过期、未被吊销）
  Y 记录TLS配置变更
